...
export default function CoursesFilter({ initialCourses, categories }: CoursesFilterProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("all");
  const [selectedLevel, setSelectedLevel] = useState("all");

  // Parse tags from JSON string
  const coursesWithParsedTags = useMemo(() => {
  ...
  const filteredCourses = useMemo(() => {
    return coursesWithParsedTags.filter((course) => {
      const matchesSearch =
  ...
  }, [searchQuery, selectedCategory, selectedLevel, coursesWithParsedTags]);
